...
```
SDL_Event event;
event.type == SDL_MOUSEMOTION    //鼠标移动
event.type == SDL_MOUSEBUTTONDOWN    //鼠标按下
    event.button.button == SDL_BUTTON_LEFT    //鼠标左键按下
event.type == SDL_MOUSEBUTTONUP    //鼠标抬起
```
例子:
例子中制作了一个按钮, 当鼠标移动到上面, 按下, 抬起后会发生变化
```
