...
uint16_t crc16_update( uint16_t crc, u16 ch )
{
  uint16_t tmp;
  uint16_t msg;
...
* 名  称：CRC16
* 功  能：计算CRC校验
* 参  数：puchMsg为要计算数组的起始指针地址，，usDataLen为数据长度
* 返回值：计算出的CRC校验，校验位16位
*/
...
* 名  称：GetLRC
* 功  能：计算LRC校验
* 参  数：pSendBuf要计算数组的起始指针地址16进制，，Len为数据长度
* 返回值：计算出的CRC校验，校验位16位
*/
...
u8 Crc8( u8 *p, u8 len )
{
    ...
    for( ; len > 0; len-- )
    {
		n = DataChange( *p );         //SHT1x  SHT2x数据需要调换高低位再做校验
        crc8 = crc_array[ crc8 ^ n ]; //查表得到CRC码
        p++;
    }
	crc8 = DataChange( crc8 );
    return crc8;
}
